...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!filter_var($price, FILTER_VALIDATE_FLOAT) && !empty($price)) {
        $priceErr = "Only decimal numbers are allowed";
        $oof = 0;
    }
    if (array_key_exists('size', $_POST)) {
        // $size = test_input($_POST['size']);
        if (!filter_var($size, FILTER_VALIDATE_FLOAT) && !empty($size)) {
            $sizeErr = "Only decimal numbers are allowed";
            $oof = 0;
        } else {
            $diff = $size;
        }
        ...
        // $weight = test_input($_POST['weight']);
        if (!filter_var($weight, FILTER_VALIDATE_FLOAT) && !empty($weight)) {
            $weightErr = "Only decimal numbers are allowed";
            $oof = 0;
        } else {
            $diff = $weight;
        }
        ...
        // echo "oof";
        if (!filter_var($height, FILTER_VALIDATE_FLOAT) && !empty($height)) {
            // echo "oof";
            $heightErr = "Only decimal numbers are allowed";
            $oof = 0;
        } else if (!filter_var($width, FILTER_VALIDATE_FLOAT) && !empty($width)) {
            $widthErr = "Only decimal numbers are allowed";
            $oof = 0;
        } else if (!filter_var($length, FILTER_VALIDATE_FLOAT) && !empty($length)) {
            $lengthErr = "Only decimal numbers are allowed";
            $oof = 0;
        } else {
